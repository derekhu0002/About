	...
	try {
		...
		if (checkIfEmpty(package) == 0) {
			try {
				...
				Session.Output("new package doc fail!" + e);
			}
		...
		if ((package.Name == "Interface") || (package.Name == "接口描述")) {
			try {
				...
				Session.Output("new doc interface fail!" + e);
			}
		...
		try {
				...
				Session.Output("new doc diagram fail!" + e);
			}
	} catch (e) {
...
function processDiagramObjects(diagram, docGenerator, packageLevel, elementSet) {
	...
	for (var i = 0; i < diagramObjects.Count; i++) {
		...
		Session.Output(currentElement.Name +  " diagramObject Sequence : " + diagramObject.Sequence);
		...
		for (var j = 0; j < currentElement.BaseClasses.Count; j++) {
			...
			if (validCount == 0) {
				docGenerator.InsertText("Parent Classes:\n", "Bold");
			}
